...
CREATE VIEW prodreadylotsview AS
	...
	LEFT JOIN [EXCELP&D].[dbo].LotOptions AS LP ON LO.[Lot ID] = LP.[lotOptionID]
	WHERE (([Room ID] NOT IN ('Throughout', 'Balance of House')) OR (([Room ID] IN ('Throughout', 'Balance of House')) AND (LP.hasThroughoutLot = 1 OR LP.lotOptionID IS NULL)))
			AND (LO.[Job IDFK] IS NOT NULL AND (JO.prodReady = 1 OR JO.prodReady IS NULL));